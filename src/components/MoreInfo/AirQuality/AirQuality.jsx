      ...
      {isLoading ? <p>Loading....</p> : !fetchData ? null : (
        <div className="flex flex-col gap-y-3">
          ...
          <div className={`relative w-full h-3 rounded-full ${width == 100 ? 'bg-green-300/70' : width >= 60 ? 'bg-yellow-300/70' : width >= 20 ? 'bg-orange-300/70' : 'bg-red-300/70' }`}>
            ...
            <span style={{left: `${width - 3}%`}}  className={`transition-all duration-1000  w-6 h-6 rounded-full border-2 block absolute -top-1.5 ${width == 100 ? 'bg-green-500' : width >= 60 ? 'bg-yellow-500' : width >= 20 ? 'bg-orange-500' : 'bg-red-500' }`}></span>
          </div>
          <div>
