    ...
    <>
      {isLoading ? <p>Loading...</p> : !fetchData ? null : (
        <div className='grid grid-cols-3 items-center  bg-light-secondary dark:bg-dark-secondary rounded-xl 2xl:rounded-3xl px-2 2xl:px-5 shadow-sm'>
            <img src={fetchData.current.condition.icon} className='w-20 md:w-24 h-20 md:h-24'/>
            ...
            <h2 className='text-6xl md:text-7xl font-NunitoBold text-right'>{Math.round(fetchData.current.temp_c)}°</h2>
        </div>
      )}
    </>
  )
